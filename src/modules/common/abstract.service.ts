...
export abstract class AbstractService<T> {
  ...
  async findBy(condition, relations = []): Promise<T | undefined> {
    try {
      ...
      throw new InternalServerErrorException(
        `Something went wrong while searching for a list of elements with condition : ${condition}.`,
      )
    }
  }
  ...
  async findById(id: string, relations = []): Promise<T> {
    try {
      ...
      Logging.error(error)
      throw new InternalServerErrorException(
        `Something went wrong while searching for a list of elements with an id : ${id}.`,
      )
    }
  }
